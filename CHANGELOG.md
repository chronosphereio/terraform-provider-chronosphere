...
Deprecated:
* Deprecate `active` field in `chronosphere_drop_rule` resource in favor of `mode` field. This field being set to `false` is disallowed and is a breaking change which requires switching to using the `mode` field.
